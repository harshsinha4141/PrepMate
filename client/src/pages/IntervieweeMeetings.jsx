...
import {
  ...
  const IntervieweeMeetings = () => {
    ...
    const handleChatClick = (meeting) => {
      ...
      if (isChatOpen && (selectedMeeting?.meetingId || selectedMeeting?._id) === key) {
        setIsChatOpen(false);
                    ...
                    <div className="space-y-3">
                      <div className="flex items-center gap-3">
                        <div className="w-12 h-12 rounded-full bg-gradient-to-r from-cyan-500 to-blue-500 flex items-center justify-center text-white font-bold text-lg flex-shrink-0 shadow-lg shadow-cyan-500/50">
                          {(m.interviewerId?.userId?.firstName || m.interviewerId?.firstName || "U")?.[0]}
                          {(m.interviewerId?.userId?.lastName || m.interviewerId?.lastName || "N")?.[0]}
                        </div>
                        <div className="flex-1 min-w-0">
                          <h3 className="text-lg font-bold text-slate-100 group-hover:bg-gradient-to-r group-hover:from-cyan-400 group-hover:to-blue-400 group-hover:bg-clip-text group-hover:text-transparent transition-all">
                            {m.interviewerId?.userId?.firstName || m.interviewerId?.firstName || "Unknown"} {" "}
                            {m.interviewerId?.userId?.lastName || m.interviewerId?.lastName || "User"}
                          </h3>
                          <div className="flex items-center gap-1 text-sm text-slate-400">
                            <Mail className="w-3 h-3 flex-shrink-0" />
                            <span className="truncate">{m.interviewerId?.userId?.email || m.interviewerId?.email || "No email"}</span>
                          </div>
                        </div>
                      </div>

                      {(m.interviewerId?.userId?.domain || m.interviewerId?.domain) && (
                        <div className="flex items-center gap-2 text-sm bg-purple-900/30 rounded-lg p-2 border border-purple-700/50">
                          <Briefcase className="w-4 h-4 text-purple-400" />
                          <span className="font-medium text-slate-300">{m.interviewerId?.userId?.domain || m.interviewerId?.domain}</span>
                        </div>
                      )}
                      ...
                      {m.timeSlot && (
                        <div className="flex items-center gap-2 text-sm text-slate-300 bg-slate-900/50 rounded-lg p-3 flex-wrap border border-slate-700">
                          ...
                          <span className="font-medium">{new Date(m.timeSlot).toLocaleDateString("en-US", { weekday: "short", year: "numeric", month: "short", day: "numeric" })}</span>
                          <Clock className="w-4 h-4 text-blue-400 ml-2" />
                          <span className="font-medium">{new Date(m.timeSlot).toLocaleTimeString("en-US", { hour: "2-digit", minute: "2-digit" })}</span>
                        </div>
                      )}
                      ...
                      <div className="flex gap-3 pt-4 border-t border-slate-700">
                        ...
                        >
                          ...
                          {chatExistsMap[m._id || m.meetingId] ? "Open Chat" : "Start Chat"}
                        </button>
                        <button
                          onClick={() => handleJoinMeeting(m)}
                          disabled={m.status === "completed" || !m.interviewerStarted}
                          className={`flex-1 inline-flex items-center justify-center gap-2 px-4 py-3 rounded-xl font-semibold text-sm transition-all ${
                            m.status === "completed"
                              ? "bg-slate-700 text-slate-500 cursor-not-allowed"
                        ...
                        >
                          <Video className={`w-5 h-5 ${m.status === "completed" ? "text-slate-500" : !m.interviewerStarted ? "text-yellow-400" : "text-cyan-400"}`} />
                          Join Meeting
                        </button>
                      </div>
                    </div>
